...
constexpr float kMaxDistance = 500000000.0f;
}  // namespace
...
void CameraController::zoom(float wheelDelta) {
  ...
  distance_ *= (1.0f - wheelDelta * 0.12f);
...
glm::mat4 CameraController::projectionMatrix(float aspect) const {
  ...
  const float nearPlane = std::max(0.0005f, distance_ * 0.0005f);
  const float farPlane = std::max(2000.0f, distance_ * 2000.0f);
